...
try {
    ...
    // FETCH LOGIC: Priority First (High to Low), then Date (Newest first)
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        nav h1 { 
            font-family: 'Gonzaga', 'Cinzel', serif; font-size: 2.2rem; font-weight: 900;
        ...
        nav ul a { text-decoration: none; color: var(--slate); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 2px; transition: 0.3s; }
        ...
        .btn-gold { padding: 22px 45px; background: var(--gold); color: var(--bg); border: none; font-weight: 600; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 3px; cursor: pointer; text-decoration: none; transition: var(--transition); display: inline-block; }
        ...
        .btn-gold:hover, .btn-outline:hover { background: var(--white); color: var(--bg); transform: translateY(-5px); }

        .feedback-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 40px; margin-top: 60px; }
        .feedback-item { background: var(--card-bg); padding: 50px; border: 1px solid rgba(255,255,255,0.03); display: flex; flex-direction: column; justify-content: space-between; min-height: 480px; }
        .review-content { font-family: 'Playfair Display', serif; font-size: 1.25rem; color: var(--white); font-style: italic; line-height: 1.8; margin-bottom: 30px; max-height: 280px; overflow-y: auto; padding-right: 15px; }
        .review-content::-webkit-scrollbar { width: 3px; }
        .review-content::-webkit-scrollbar-thumb { background: var(--gold); }

        .author-name { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 3px; color: var(--gold); display: flex; align-items: center; gap: 10px; }

        .glass-card { background: var(--card-bg); padding: 80px; border: 1px solid rgba(255,255,255,0.05); }
        .form-box input, .form-box textarea, .form-box select { width: 100%; padding: 20px 0; margin-bottom: 30px; border: none; border-bottom: 1px solid rgba(255,255,255,0.1); background: transparent; color: var(--white); outline: none; }
        
        .pagination { margin-top: 50px; display: flex; justify-content: center; gap: 10px; }
        .pagination a { text-decoration: none; padding: 12px 18px; border: 1px solid rgba(255,255,255,0.1); color: var(--white); font-size: 0.7rem; transition: 0.3s; }
        ...
        @media (max-width: 1100px) { .hero { flex-direction: column; text-align: center; } .hero-text h2 { font-size: 3.5rem; } nav ul { display: none; } }
    </style>
</head>
<body>
...
<section id="hero" class="hero">
    <div class="hero-text">
        ...
        <p style="font-size: 1.3rem; font-weight: 300; margin-bottom: 45px; max-width: 650px;">
            Senior Accountant & Financial Operations Partner helping global businesses gain clarity and control.
        </p>
        <div class="hero-btns">
            <a href="#connect" class="btn-gold">Secure Partnership</a>
            <a href="#" class="btn-outline"><i class="fas fa-file-download" style="margin-right: 10px;"></i>Download CV</a>
        </div>
    </div>
...
<section id="feedback" style="border-top: 1px solid rgba(255,255,255,0.05);">
    ...
    <div class="feedback-grid">
        <?php if(!empty($comments)): foreach ($comments as $row): ?>
            <div class="feedback-item">
                <div class="review-content">"<?php echo htmlspecialchars($row['comment_text']); ?>"</div>
                <div class="feedback-author">
                    <p class="author-name">
                        <?php if(!empty($row['country_code'])): ?>
                            <img src="https://flagcdn.com/w20/<?php echo strtolower(htmlspecialchars($row['country_code'])); ?>.png" width="20" alt="Flag" onerror="this.style.display='none'">
                        <?php endif; ?>
                        — <?php echo htmlspecialchars($row['name']); ?> 
                    </p>
                    <p style="margin-top: 5px; font-size: 0.7rem;">
                        <span style="color:var(--white); opacity:0.8;"><?php echo htmlspecialchars($row['position'] ?? 'Consultant'); ?></span> / <?php echo htmlspecialchars($row['company'] ?? 'N/A'); ?>
                    </p>
                </div>
            </div>
        <?php endforeach; else: ?>
    ...
    <div class="pagination">
        <?php if ($page > 1): ?><a href="?page=<?php echo $page - 1; ?>#feedback">← PREV</a><?php endif; ?>
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="?page=<?php echo $i; ?>#feedback" class="<?php echo ($i == $page) ? 'active' : ''; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>
        <?php if ($page < $total_pages): ?><a href="?page=<?php echo $page + 1; ?>#feedback">NEXT →</a><?php endif; ?>
    </div>
    <?php endif; ?>
</section>
    ...
    <div class="glass-card">
        <div class="form-box">
            <form action="send_email.php" method="POST">
                <h3 style="font-family: 'Playfair Display', serif; font-size: 3.5rem; color: white; text-align:center;">Let's Connect</h3>
                ...
                <textarea name="message" rows="4" placeholder="How can I assist?" required></textarea>
...
<section style="background: rgba(255, 255, 255, 0.02); border-top: 1px solid rgba(255,255,255,0.05);">
    <div style="max-width: 700px; margin: 0 auto; text-align: center;">
        ...
        <form id="reviewForm" class="form-box">
            ...
            <textarea name="comment_text" rows="3" placeholder="Write testimonial..." required></textarea>
            <button type="submit" id="reviewBtn" class="btn-gold">Post Review</button>
        </form>
        <p id="submissionNote" style="margin-top: 15px; color: var(--gold); display: none;">Submitted for admin approval.</p>
    </div>
</section>

<footer>
    <p>PH: +63 999 586 6190 | E: afryllou.consulting@gmail.com</p>
    <p style="margin-top: 20px;">&copy; <?php echo date("Y"); ?> AFRYL LOU OKIT.</p>
</footer>
    ...
    document.getElementById('reviewForm').addEventListener('submit', function(e) {
        ...
        .then(response => {
            if(response.ok) {
                ...
                alert("Submission failed."); btn.disabled = false; btn.innerHTML = 'Post Review';
            }
        });
    });
</script>
